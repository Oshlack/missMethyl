...
\date{Modified: 14 November, 2014. Compiled: \today}
...
Now that we have performed an initial differential methylation analysis to rank the CpGs with respect to their association with the factor of interest, we can designate the CpGs that are least associated with the factor of interest based on FDR-adjusted p-value as negative control features.  

<<ruv1>>=
topTab <- topTable(fit, coef=2, num=Inf)
head(topTab)

ctl <- rownames(M) %in% rownames(topTab[topTab$adj.P.Val > 0.5,])
table(ctl)
@

We can now use the empirical controls to perform a differential methylation analysis using (by default) the RUV-inverse method, which is adjusted for the unwanted variation estimated from the data.

<<ruv2, cache=TRUE>>=
# Perform RUV adjustment and fit
rfit <- RUVfit(data=M, design=des, coef=2, ctl=ctl)
rfit2 <- RUVadj(rfit)
