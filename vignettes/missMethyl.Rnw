...
\date{Modified: 4 September, 2014. Compiled: \today}
...
par(mfrow=c(2,2))
for(i in 1:4){
stripchart(beta[rownames(beta)==cpgsDV[i],]~design2[,2],method="jitter",
group.names=c("Normal","Cancer"),pch=16,cex=1.5,col=c(4,2),ylab="Beta values",
vertical=TRUE,cex.axis=1.5,cex.lab=1.5)
...
fitvar.hapmap <- varFit(y, design.hapmap)
...
\begin{figure}[htbp!]
\centering
<<top4DVhapmap,fig=TRUE,width=10,height=10>>=
