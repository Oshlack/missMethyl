...
variation to remove) where *0 <= k < no. samples*.
...
using an MDS plot for example, the `getAdjusted` function can be used to extract 
...
```{r mdsplotadj,fig.height=5,fig.width=9,fig.cap="An MDS plot of cancer and normal data, before and after RUVm adjustment."}
par(mfrow=c(1,2))
plotMDS(Mval, labels=targets$Sample_Name, col=as.integer(factor(targets$status)),
        main="Unadjusted")
...
To illustrate how the `getAdjusted` function can be used to help select an
...
```{r mdsplotadj1,fig.height=9,fig.width=9,fig.cap="MDS plots of cancer and normal data before an after adjustment with RUV-inverse and RUV-4"}
par(mfrow=c(2,2))
plotMDS(Mval, labels=targets$Sample_Name, col=as.integer(factor(targets$status)),
        main="Unadjusted")
legend("topleft",legend=c("Cancer","Normal"),pch=16,cex=1,col=1:2)
