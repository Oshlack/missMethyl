...
The `r Biocpkg("missMethyl")` package contains functions to analyse methylation 
...
Specifically, `r Biocpkg("missMethyl")` contains functions to perform SWAN 
...
`r Biocpkg("minfi")` package [@Aryee2014]. Statistical analyses are usually 
...
by `r Biocpkg("minfi")`. For detecting differentially variable CpGs we recommend that the
analysis is performed on M-values. All analyses described here are
performed at the CpG site level.

# Reading data into R

We will use the data in the `r Biocpkg("minfiData")` package to demonstrate the 
functions in `r Biocpkg("missMethyl")`.
...
the `r Biocpkg("minfi")` User's Guide. For additional details on how to read 
the IDAT files into R, as well as information regarding quality control please 
refer to the `r Biocpkg("minfi")` User's Guide.
...
normalization method for Illumina 450k BeadChips. Technical differencs
have been demonstrated to exist between the Infinium I and Infinium II
assays on a single 450K array [@Bibikova2011, @Dedeurwaerder2011]. Using the 
SWAN method
...
To test for differential methylation we use the `r Biocpkg("limma")` 
...
`r Biocpkg("limma")` model framework can handle any experimental design which 
...
to the `r Biocpkg("limma")` User's Guide.
...
functions from `r Biocpkg("limma")`. As input data we use the matrix of 
M-values.
...
displayed using the `decideTests` function in `r Biocpkg("limma")` and the top 
...
`r Biocpkg("missMethyl")` provides a `r Biocpkg("limma")`-like interface to 
functions from the CRAN package `r CRANpkg("ruv")` that
...
that as a default, `varFit` uses the robust setting in the `r Biocpkg("limma")` 
framework, which requires the use of the `r CRANpkg("statmod")` package.
...
To demonstrate this, we use data from the `r Biocpkg("tweeDEseqCountData")` 
...
males and females. We follow the code from the `r Biocpkg("limma")` vignette to 
...
using the `r Biocpkg("edgeR")` package.
...
`r Biocpkg("goseq")` method of Young et al. [@Young2010]. Each gene then has a 
...
`r Biocpkg("goana")` and `r Biocpkg("kegga")` are
...
`r Biocpkg("org.Hs.eg.db")` package.
